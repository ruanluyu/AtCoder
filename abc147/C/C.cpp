...
int main(){
	...
	REP(i,ctm){
		...
		REP1(j,n){
			if( (1<<(j-1)) & i ){
				...
				ITE(lst[j]){
					ll id = ite->P1;
					ll cont = ite->P2;
					DBPRT(((1<<(id-1))&i));DBPRT((cont<<(id-1)));
					if(((1<<(id-1))&i)!=(cont<<(id-1))) {contr = 1;DBPRT(i);DBPRT(j);DBPRT((ite-lst[i].begin()));break;}
				}
			} else continue;
			if(contr) break;
		}
		if(!contr){
			DBSTART
			DBPRT("Suc");DBPRT(i);ENTER;
			DBEND
			ll curres = 0;
			REP(j,n) if((1<<j) & i) curres ++;
			res = max(res,curres);
		}
	}
