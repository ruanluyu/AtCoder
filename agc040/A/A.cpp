...
int main(){
	...
	MINUS(a);
	vector<ll> ids;
	if(s[0]=='<') {a[0] = 0;ids.PB(0);}else a[0] = -2;
	if(s[sz-1]=='>') {a[sz] = 0;ids.PB(sz);} else a[sz] = -2;
	
	
	REP(i,sz-1){
		if(s[i]=='>' && s[i+1] == '<') {a[i+1]=0; ids.PB(i+1); continue;}
		if(s[i]=='<' && s[i+1] == '>') a[i+1]=-2;
	}
	//DBPRT(ids);
	ITE(ids){
		ll id = *ite;
		FORE(i,id,sz){
			if(a[i]==-2) {a[i] = i-id; break;}
			a[i] = i-id;
		}
		for(ll i = id;i>=0;i--){
			if(a[i]>0){
				a[i] = max(a[i],id-i);break;
			}
			a[i] = id-i;
		}
	}
