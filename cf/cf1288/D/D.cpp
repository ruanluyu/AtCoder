...
int main(){
	...
	ll i1=1,i2=1;
	ll curv = INF;
	REP1(i,m) MIN(curv,a[1][i]);
	REP1(i,n){
		ll b[MAXM];
		ZERO(b);
		REP1(j,m) b[j] = max(a[i1][j],a[i][j]);
		PRTLST(b,m+1);ENTER;
		ll curl = INF;
		REP1(j,m) MIN(curl,b[j]);
		if(curl>curv) {i2 = i;curv=curl;continue;}

		REP1(j,m) b[j] = max(a[i2][j],a[i][j]);
		PRTLST(b,m+1);ENTER;
		curl = INF;
		REP1(j,m) MIN(curl,b[j]);
		if(curl>curv) {i1 = i;curv=curl;continue;}
