...
int main(){
	...
	ll lower=0,upper=x;
	ll res;
	if(lowerx(lower)) res = 0;
	else
		do{
			if(lower == upper) {res = lower;break;}
			if(upper-lower == 1) {res = lowerx(upper)?upper:lower;break;}
			ll thisv = (lower+upper)/2;
			if(lowerx(thisv)) upper = thisv;
			else lower = thisv;
		}while(1);
