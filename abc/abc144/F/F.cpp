...
int main(){
	...
	REP(i,m) {ll s,t; cin >> s >> t; G[s].PB(t);}
	ZERO(dp);
	dp[n-1] = 1;
	ll maxid[MAXN];
	double summem[MAXN];
	ZERO(maxid);ZERO(summem);
	PER1(i,n-2){
		double sum = 0;
		ll sz = G[i].size();
		double maxdp = -1;
		ITE(G[i]){
			sum += dp[*ite];
			if(dp[*ite]>maxdp) {maxid[i] = *ite;maxdp = dp[*ite];}
		}
		dp[i] = sum / sz + 1;
		summem[i] = sum;
	}
	...
	REP1(i,n-1) {
		...
		if(sz == 1){
			...
			double cur = (summem[i]-dp[maxid[i]])/(sz-1)+1;
			if(cur>=dp[i]) continue;
			dp[i] = cur;
		}
		...
		PER1(j,i-1)
		{
			double s = 0;
			ll cursz = 0;
			ITE(G[j]) if(dp[*ite]<INF) {s+=dp[*ite];cursz++;}
			dp[j] = s/cursz+1;
		}
		res = min(res,dp[1]);
		DBSTART
